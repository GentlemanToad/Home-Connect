<?php
	...
	{
		$pass = trim($password); //remove white space and predefined characters from entered password
		include 'connect.php';  //connect to database
		...
		if($rcount>0) //checks to see if an entry has been returned
		{
			...
			if(($stusercount>0) AND ($attemptnum<3)) //checks to see if an entry has been returned
			{
				if(password_verify($pass,$strow['Password'])) //verifies stored hashed password and entered password are the same
				{
				...
				else
				{
					...
					if($attemptnum<2)
					{
						...
						$_SESSION["id_loggedIn"]=null; //id_loggedIn session is assigned a null as user's password entered did not match
